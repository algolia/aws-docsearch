...
+ [Amazon S3 Multipart Upload Limits](qfacts.mdx)
+ [API Support for Multipart Upload](sdksupportformpu.mdx)
+ [Multipart Upload API and Permissions](mpuAndPermissions.mdx)

The Multipart upload API enables you to upload large objects in parts\. You can use this API to upload new large objects or make a copy of an existing object \(see [Operations on Objects](ObjectOperations.mdx)\)\. 
...
For more information about the bucket lifecycle configuration, see [Object Lifecycle Management](object-lifecycle-mgmt.mdx)\.
...
1.  Set up the AWS CLI\. For instructions, see [Setting Up the AWS CLI](setup-aws-cli.mdx)\. 
