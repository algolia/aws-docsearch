...
1. Create an IAM role and assign permissions\. This role grants Amazon S3 permission to add object tags, for which you create a job in the next step\.
   ...
   1. Create an IAM policy with permissions and attach it to the IAM role that you created in the previous step\. For more information about permissions, see [Granting Permissions for Amazon S3 Batch OperationsRequired Permissions for Creating an Amazon S3 Batch Operations Job](batch-ops-iam-role-policies.mdx)\.
