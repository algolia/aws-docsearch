...
| acl |  An Amazon S3 access control list \(ACL\)\. If an invalid access control list is specified, an error is generated\. For more information on ACLs, see [Access Control Lists](Introduction.mdx#S3_ACLs)\. Type: String Default: private  Valid Values: `private \| public-read \| public-read-write \| aws-exec-read \| authenticated-read \| bucket-owner-read \| bucket-owner-full-control `   |  No  |
