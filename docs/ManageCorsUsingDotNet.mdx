...
To manage cross\-origin resource sharing \(CORS\) for a bucket, you can use the AWS SDK for \.NET\. For more information about CORS, see [Cross\-Origin Resource Sharing \(CORS\)](cors.mdx)\.
...
For information about creating and testing a working sample, see [Running the Amazon S3 \.NET Code Examples](UsingTheMPDotNetAPI.mdx#TestingDotNetApiSamples)\.   
