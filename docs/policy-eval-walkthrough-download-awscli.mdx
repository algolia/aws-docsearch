...
The introductory examples \(see [Example Walkthroughs: Managing Access to Your Amazon S3 Resources ](example-walkthroughs-managing-access.mdx)\) use the AWS Management Console to create resources and grant permissions\. And to test permissions, the examples use the command line tools, AWS Command Line Interface \(CLI\) and AWS Tools for Windows PowerShell, so you don't need to write any code\. To test permissions, you must set up one of these tools\. 
...
For a list of commands, go to [Amazon Simple Storage Service .mdxlets](https://docs.aws.amazon.com/powershell/latest/reference/Index.html)\. 
