---
id: kms-using-sdks
title: AWS Key Management Service in Amazon S3 Using the AWS SDKs
---
...
In this case, Amazon S3 uses the AWS managed CMK \(see [Using Server\-Side Encryption with CMKs Stored in AWS KMS ](UsingKMSEncryption.mdx)\)\. You can optionally create a symmetric customer managed CMK and specify that in the request\.
...
+ For uploading an object in a single operation, see [Upload an Object Using the AWS SDK for Java](UploadObjSingleOpJava.mdx)\.
+ For a multipart upload, see the following topics:
  + Using high\-level multipart upload API, see [Upload a File](HLuploadFileJava.mdx)\. 
  + If you are using the low\-level multipart upload API, see [Upload a File](llJavaUploadFile.mdx)\.

### Copy Operation<a name="kms-using-sdks-java-copy"></a>

When copying objects, you add the same request properties \(`ServerSideEncryptionMethod` and `ServerSideEncryptionKeyManagementServiceKeyId`\) to request Amazon S3 to use an AWS KMS CMK\. For more information about copying objects, see [Copying Objects](CopyingObjectsExamples.mdx)\.
...
For a code example, see [Generate a presigned Object URL Using the AWS SDK for Java](ShareObjectPreSignedURLJavaSDK.mdx)\. 
...
In this case, Amazon S3 uses the AWS managed CMK\. For more information, see [Protecting Data Using Server\-Side Encryption with CMKs Stored in AWS Key Management Service \(SSE\-KMS\)](UsingKMSEncryption.mdx)\. You can optionally create your own symmetric customer managed CMK and specify that in the request\. 
...
+ For uploading an object in a single operation, see [Upload an Object Using the AWS SDK for \.NET](UploadObjSingleOpNET.mdx)\.
+ For multipart upload see the following topics:
  + Using high\-level multipart upload API, see [Upload a File to an S3 Bucket Using the AWS SDK for \.NET \(High\-Level API\)](HLuploadFileDotNet.mdx)\. 
  + Using low\-level multipart upload API, see [Upload a File to an S3 Bucket Using the AWS SDK for \.NET \(Low\-Level API\)](LLuploadFileDotNet.mdx)\.
...
For a code example, see [Generate a Presigned Object URL Using AWS SDK for \.NET](ShareObjectPreSignedURLDotNetSDK.mdx)\.