...
+ **Actions** – For each resource, Amazon S3 supports a set of operations\. You identify resource operations that you will allow \(or deny\) by using action keywords \(see [Specifying Permissions in a Policy](using-with-s3-actions.mdx)\)\. 
...
+ [Specifying Resources in a Policy](s3-arn-format.mdx)
+ [Specifying a Principal in a Policy](s3-bucket-user-policy-specifying-principal-intro.mdx)
+ [Specifying Permissions in a Policy](using-with-s3-actions.mdx)
+ [Specifying Conditions in a Policy](amazon-s3-policy-keys.mdx)

The following topics provide additional policy examples:
+ [Bucket Policy Examples](example-bucket-policies.mdx)
+ [User Policy Examples](example-policies-s3.mdx)