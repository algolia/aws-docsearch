...
This section provides an example of listing object versions from a versioning\-enabled bucket\. Amazon S3 stores object version information in the *versions* subresource \(see [Bucket Configuration Options](UsingBucket.mdx#bucket-config-options-intro)\) that is associated with the bucket\. 
