...
1. Grant Permissions\. 
   ...
   1. In the Amazon S3 console, attach the following bucket policy to *examplebucket*\. 
      ...
      In the `Principal` statement, Dave is identified by his user ARN\. For more information about policy elements, see [Access Policy Language Overview](access-policy-language-overview.mdx)\.
...
1. Update the AWS CLI config file by adding the following UserDaveAccountA profile\. For more information, see [Setting Up the Tools for the Example Walkthroughs](policy-eval-walkthrough-download-awscli.mdx)\.
