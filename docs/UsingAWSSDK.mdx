...
+ [Setting Up the AWS CLI](setup-aws-cli.mdx)
+ [Using the AWS SDK for Java](UsingTheMPJavaAPI.mdx)
+ [Using the AWS SDK for \.NET](UsingTheMPDotNetAPI.mdx)
+ [Using the AWS SDK for PHP and Running PHP Examples](UsingTheMPphpAPI.mdx)
+ [Using the AWS SDK for Ruby \- Version 3](UsingTheMPRubyAPI.mdx)
+ [Using the AWS SDK for Python \(Boto\)](UsingTheBotoAPI.mdx)
+ [Using the AWS Mobile SDKs for iOS and Android](using-mobile-sdks.mdx)
+ [Using the AWS Amplify JavaScript Library](using-aws-amplify.mdx)
...
To get the version information, use the `Get-Module` .mdxlet: 
...
To update the 3\.3\.0\.0 version, use the `Update-Module` .mdxlet: 
